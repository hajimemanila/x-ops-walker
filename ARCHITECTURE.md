v2.2系のアーキテクチャ変遷、および直近で完了した「ALMデータモデルの一極集中化（DRY原則の適用と技術的負債の排除）」を完全に反映した `ARCHITECTURE.md` の最終版を出力します。

既存のロジックや思想を1バイトも削ることなく、現在のシステムの実態（事実）と完全に同期させています。そのままプロジェクトルートに配置してください。

---

# X-Ops Walker: Architecture & Contributor Guide (v2.2.3)
...
- **v2.2.x**: JIT空間ナビゲーション基盤の確立、Phantom/ALM Stateの一元管理化（`config/state.ts`への集約）、Options.htmlへの設定集約、およびGemini Walker等の新規プロトコル実装 ※完了済
...
- **Storage の区画整理と階層化 (State Centralization)**: `chrome.storage.local` のデータ構造はドメイン・機能ごとに明確に階層化して分離し、その型定義とデフォルト値はすべて `src/config/state.ts` に一極集中（Centralize）させる。
...
```
graph TD
    Config[src/config/state.ts<br>Single Source of Truth] -.->|Types & Defaults| Storage Area
    Config -.->|Imports| UI
    Config -.->|Imports| Background
...
- **ハードコードの禁止**: 設定のキー名やデフォルト配列を直接ファイルに書かないでください。必ず `config/state.ts` に定義してインポートすること。
