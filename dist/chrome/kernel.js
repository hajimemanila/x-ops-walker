...
(() => {
  ...
  function getCurrentTarget(selector, focusClass = "x-walker-focused", container) {
  ...
  function focusNextTarget(selector, direction, offset = 0, focusClass = "x-walker-focused", container) {
    const targets = getValidTargets(selector);
    if (targets.length === 0) return null;
...
`;
      ...
      if (key === "m" && !shift) {
        ...
        const currentEl = getCurrentTarget(TARGET_SELECTOR2, FOCUS_CLASS, container);
